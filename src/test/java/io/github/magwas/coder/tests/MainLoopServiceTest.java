...
public class MainLoopServiceTest extends TestBase implements MainLoopTestData {
	...
	void testExitCommand() throws Exception {
		...
		assertThrows(SecurityException.class, () -> underTest.apply());
	}
	...
	void testHistoryCommand() throws Exception {
		...
		verify(systemDependency).println(HISTORY_MESSAGE + 5);
	}

	@Test
	@DisplayName("Instructions command shows status")
	void testInstructionsCommand() throws Exception {
	...
	void testProcessingError() throws Exception {
		...
		underTest.apply();
		verify(openRouterClientService).apply(ANY_QUESTION);
		verify(systemDependency).println("Error: Simulated error");
	}
